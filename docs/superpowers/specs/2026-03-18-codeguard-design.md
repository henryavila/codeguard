...
The bundle is pre-compiled during `npm run build` as a tsdown entry point with ``noExternal: [/.*/]``. Ships as `dist/hooks/runner.js` in the npm package. During `/codeguard-setup`, the AI copies it to `.codeguard/hook-runner.js`. No runtime compilation needed.
...
- **Adapters bundled in** — pre-compiled during `npm run build` with ``noExternal: [/.*/]``, setup copies the bundle
